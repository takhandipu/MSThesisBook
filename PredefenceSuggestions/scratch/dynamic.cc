...
int totalPacketsPerSU = 1024 * 1; //per SU 1 MB data
...
class RadioAllocatedChannels
{
public:
    int totalAllocatedChannels;
    int currentAllocatedChannelIndex;
    vector<int> allocatedChannel;
    RadioAllocatedChannels(vector<int> list)
    {
        this->totalAllocatedChannels = list.size();
        this->allocatedChannel=list;
        this->currentAllocatedChannelIndex=0;
    }
    int getCurrentChannel()
    {
        return this->allocatedChannel[this->currentAllocatedChannelIndex];
    }
    int getNextChannel()
    {
        this->currentAllocatedChannelIndex+=1;
        this->currentAllocatedChannelIndex%=totalAllocatedChannels;
        return this->allocatedChannel[this->currentAllocatedChannelIndex];
    }
};
...
public:
    ...
    RadioAllocatedChannels *dynamicAllocations[MAX_CHANNELS];
    ...
    void startSwitching(int radioIndex, Ptr<Packet> p)
    {
        if(alwaysSwitch)
        {
            int changedChannelIndex = dynamicAllocations[radioIndex]->getNextChannel();
            char debugString[80];
    ...
    MySUNode(int nRadios=1)
    {
        ...
        int channelPerRadio = MAX_CHANNELS/numberOfRadios;
        int extras = MAX_CHANNELS%numberOfRadios;
        int channelIndex = 0;
        for(int i = 0; i < numberOfRadios; i++)
        {
            ...
            vector<int> list;
            for(int j = 0; j < channelPerRadio; j++)
            {
                list.push_back(channelIndex);
                channelIndex++;
            }
            if(i<extras)
            {
            ...
            this->dynamicAllocations[i]=new RadioAllocatedChannels(list);
            
            int initialChannelIndex = dynamicAllocations[i]->getCurrentChannel();
