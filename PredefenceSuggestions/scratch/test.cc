...
public:
  ...
  PsrExperiment ();
  void change(Ptr<YansWifiChannel> channel, Ptr<YansWifiPhy> tx, Ptr<YansWifiPhy> rx){
...
{
  m_output.received++;
  std::cout<<m_tx->GetChannelNumber()<<std::endl;
}
...
PsrExperiment::Run (struct PsrExperiment::Input input)
{
  ...
  channel2->SetPropagationDelayModel (CreateObject<ConstantSpeedPropagationDelayModel> ());
  Ptr<LogDistancePropagationLossModel> log2 = CreateObject<LogDistancePropagationLossModel> ();
  channel2->SetPropagationLossModel (log2);
  ...
  for (uint32_t i = 0; i < m_input.nPackets; ++i)
    {
      ...
      if(i==49)Simulator::Schedule (Seconds (i), &PsrExperiment::change, this, channel2, tx, rx);
    }
  m_tx = tx;
  Simulator::Run ();
  Simulator::Destroy();
