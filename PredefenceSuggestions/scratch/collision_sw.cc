...
public:
    ...
    void startSwitching(int radioIndex, Ptr<Packet> p, int starvationAvoid=0)
    {
        if(alwaysSwitch)
        {
            int changedChannelIndex = randomChannelAllocator->GetInteger() % MAX_CHANNELS;
            if(channelAllcoation[changedChannelIndex]==true)
            {
                if(starvationAvoid<5){
                    Simulator::Schedule (Seconds (switchTime), &MySUNode::startSwitching, this, radioIndex, p, starvationAvoid+1);
                }
                return;
            }
            char debugString[80];
    ...
    void startSensing(int radioIndex, Ptr<Packet> p)
    {
        ...
        if(senseResult==true)
        {
            //PU is present
            //schedule switching
            Simulator::Schedule (Seconds (0.0), &MySUNode::startSwitching, this, radioIndex, p, 0);
        } else {
            //PU is not present
