...
double packetInterval = 1.0/32.0; //data injection 32 KBps, 256Kbps
int totalPacketsPerSU = 1024 * 10; //per SU 10 MB data
...
public:
    ...
    void startSwitching(int radioIndex, Ptr<Packet> p)
    {
        if(alwaysSwitch)
        {
            ...
            int changedChannelIndex = freeChannelIndexes[randValue];
