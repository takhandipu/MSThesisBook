...
public:
    ...
    map<int64_t, bool> packetsReceivedInNanoSecond;
        ...
        for(int i = 0; i < numberOfRadios; i++)
        {
            if(tag.GetFlowId () == m_flowIds[i])
            {
                ...
                if(timeTag==0)
                {
                    ...
                    int64_t checkedTime = txTime.GetNanoSeconds();
                    map<int64_t, bool>::iterator notFirstPacket = packetsReceivedInNanoSecond.find(checkedTime);
                    if(notFirstPacket==packetsReceivedInNanoSecond.end()){ 
                        delaySum += Now()-txTime;
                        //packetsReceivedPerRadio[i]++;
                        timeLastRxPacket = Now();
                        packetsReceivedInNanoSecond.insert(make_pair(checkedTime, true));
                    } else {
                        //redundent packet
                    }
                }
