...
double packetInterval = 40.0/1024.0; //data injection 1 MBps, 8Mbps
int totalPacketsPerSU = 1024 * 10; //per SU 1 MB data
...
public:
    ...
    void startSensing(int radioIndex, Ptr<Packet> p)
    {
        ...
        if(senseResult==true)
        {
            //PU is present
            //schedule switching
            NS_LOG_DEBUG(Now()<<" Sensed Primary User "<<radioIndex<<" "<<currentChannel[radioIndex]);
            Simulator::Schedule (Seconds (senseTime), &MySUNode::startSwitching, this, radioIndex, p);
        } else {
            //PU is not present
            //schedule transmission
            NS_LOG_DEBUG(Now()<<" Sensed No Primary User "<<radioIndex<<" "<<currentChannel[radioIndex]);
        ...
        for(int i = 0; i < numberOfRadios; i++)
        {
            if(tag.GetFlowId () == m_flowIds[i])
            {
                ...
                if(timeTag==0)
                {
                    ...
                    //NS_LOG_DEBUG("Send"<<txTime);
                    //NS_LOG_DEBUG("Receive"<<Now());
                    delaySum += Now()-txTime;
                    NS_LOG_DEBUG("Delay "<<Now()-txTime<<" "<<currentChannel[i]);
