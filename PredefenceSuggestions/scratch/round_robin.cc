...
public:
    ...
    unsigned int getNextChannel()
    {
        unsigned int nextValue = nextChannel;
        nextChannel += 1;
        nextChannel %= MAX_CHANNELS;
        return nextValue;
    }
    ...
    void startSwitching(int radioIndex, Ptr<Packet> p)
    {
        if(alwaysSwitch)
        {
            ...
            {
                nextIndex = getNextChannel();
                if(channelAllcoation[nextIndex]==true)continue;
                else {
                     isFound = true;
                     break;
                }
            ...
            int changedChannelIndex = nextIndex;
